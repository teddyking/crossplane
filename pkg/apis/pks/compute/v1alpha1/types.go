...
// Defaults for PKS resources.
const (
	DefaultReclaimPolicy = corev1alpha1.ReclaimRetain
	DefaultNumberOfNodes = int64(1)
)

// PKSClusterSpec specifies the configuration of a PKS cluster.
type PKSClusterSpec struct {
	...
	Scopes               []string `json:"scopes,omitempty"`                   //--scopes=[SCOPE,…]; default="pks-default"
...
// PKSClusterStatus represents the status of a PKS cluster.
type PKSClusterStatus struct {
...
// PKSCluster is the Schema for the instances API
// +k8s:openapi-gen=true
// +groupName=compute.pks
...
type PKSCluster struct {
	...
	Spec   PKSClusterSpec   `json:"spec,omitempty"`
	Status PKSClusterStatus `json:"status,omitempty"`
}

// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object

// PKSClusterList contains a list of PKSCluster items
type PKSClusterList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []PKSCluster `json:"items"`
}

// NewPKSClusterSpec from properties map
func NewPKSClusterSpec(properties map[string]string) *PKSClusterSpec {
	spec := &PKSClusterSpec{
...
func (g *PKSCluster) ObjectReference() *corev1.ObjectReference {
	return util.ObjectReference(g.ObjectMeta, util.IfEmptyString(g.APIVersion, APIVersion), util.IfEmptyString(g.Kind, PKSClusterKind))
}

// OwnerReference to use this instance as an owner
func (g *PKSCluster) OwnerReference() metav1.OwnerReference {
	return *util.ObjectToOwnerReference(g.ObjectReference())
}

// ConnectionSecret returns the connection secret for this PKS cluster.
func (g *PKSCluster) ConnectionSecret() *corev1.Secret {
...
func (g *PKSCluster) ConnectionSecretName() string {
...
func (g *PKSCluster) State() string {
	return g.Status.State
}

// IsAvailable for usage/binding
func (g *PKSCluster) IsAvailable() bool {
	return g.State() == ClusterStateRunning
}

// IsBound returns true if this PKS cluster is bound to a resource claim.
func (g *PKSCluster) IsBound() bool {
	return g.Status.IsBound()
}

// SetBound specifies whether this PKS cluster is bound to a resource claim.
func (g *PKSCluster) SetBound(bound bool) {
